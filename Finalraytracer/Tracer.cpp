...
Vector3D shade(Ray& ray, int depth);
...
int main()
{
	...
	for (int y = HEIGHT - 1; y >= 0; y--)
	{
		for (int x = 0; x < WIDTH; x++)
		{
			...
			Vector3D result = shade(viewing_ray, 0);
...
Vector3D shade(Ray& ray, int depth)
{
	...
	if (plane.hit(ray))
	{
		...
		Ray reflection_ray = Ray(Point3D(hit_point.x,hit_point.y, hit_point.z), (surface_normal * -2 * surface_normal.dotProduct(ray.getDirection())) + ray.getDirection());
		...
		if (depth < 3)
		{
			return L * 0.5 + shade(reflection_ray, depth + 1) * 0.5;
		}
		return L;
	}
